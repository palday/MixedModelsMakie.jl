...
# ╔═╡ e8cebaf1-de53-4e55-a7b3-40f44d8485e1
"""
...
# ╔═╡ b00c7ab4-cc20-4ee9-b407-be15c85f2157
fm0 = fit(MixedModel, @formula(reaction ~ 1 + days + (1|subj)), MixedModels.dataset(:sleepstudy))

# ╔═╡ a346ea55-ddc9-4075-9450-e4b84f72084c
fm1 = fit(MixedModel, @formula(reaction ~ 1 + days + (1+days|subj)), MixedModels.dataset(:sleepstudy))

# ╔═╡ 4eeb6952-6a9c-476c-b1cb-c8cfab70cdc4
qqnorm(residuals(fm1))

# ╔═╡ 6ac475bb-32d8-4947-add3-6bbe5a0f7d20
let f = Figure()
	ax = Axis(f[1, 1])
	rr = residuals(fm1)
	lims = extrema(rr)
...
# ╔═╡ 54cde52d-f46c-4c65-8655-231da890e7cb
Makie.convert_arguments(P::Type{<:Makie.QQNorm}, x::MixedModel) = convert_arguments(P, residuals(x))

# ╔═╡ 6fe60e61-6aff-4942-9060-f924dc4621f8
Makie.convert_arguments(P::Type{<:Makie.QQPlot}, x::MixedModel) = convert_arguments(P, residuals(x))

# ╔═╡ 953febc3-8b97-431f-bde6-bf63074d00ac
qqnorm(fm1)

# ╔═╡ 5e02b915-4d6c-4d14-81e2-86dc2a74815f
let f = Figure()
	qqnorm!(Axis(f[1, 1]), fm1)
	f
end
...
[[ArrowTypes]]
deps = ["UUIDs"]
git-tree-sha1 = "ac464acb43b227acf91e8f695fbfdbb1a37ce1e4"
